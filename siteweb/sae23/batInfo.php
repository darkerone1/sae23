    ...
    <body>
        ...
        <section id="TableWrapper">
            <section class="monitor">
                    ...
                    <?php if($_SESSION['username'] == "jules" || $_SESSION['username'] == "admin"){ ?>
                        <table>
                            <thead>
                                <tr>
									<td>Nom_capteur</td>
                            ...
                            <tbody>
                                <?php
                                    $results = mysql_query("SELECT  `nom_capteur`, `horaire`, `valeur`, `id` FROM `mesure` 
                                                            WHERE `nom_capteur` LIKE 'D%temperature' ORDER BY `mesure`.`horaire` DESC;");
                     ...
                     <?php } else { 
                          $results = mysql_query("SELECT  `nom_capteur`, `horaire`, `valeur`, `id` FROM `mesure` 
                          WHERE `nom_capteur` LIKE 'D%temperature' ORDER BY `mesure`.`horaire` DESC LIMIT 1;");
            ...
            <section id="addTempPopup" class="popup">
                ...
                <section class="monitor">
                    <article id="CO2Table">
                        <?php if($_SESSION['username'] == "jules"  || $_SESSION['username'] == "admin"){ ?>
                            <table>
                                ...
                                <tbody>
                                    <?php
                                        $results = mysql_query("SELECT `nom_capteur`, `horaire`, `valeur`, `id` FROM `mesure` 
                                                                WHERE `nom_capteur` LIKE 'D%CO2' ORDER BY `mesure`.`horaire` DESC;");
                                        while($row = mysql_fetch_array($results)) {
                                    ?>
                                        <tr>
											<td><?php echo $row['nom_capteur']?></td>
                     ...
                     <?php } else { 
                          $results = mysql_query("SELECT  `horaire`, `valeur`, `id` FROM `mesure` 
                          WHERE `nom_capteur` LIKE 'D%CO2' ORDER BY `mesure`.`horaire` DESC LIMIT 1;");
                          $row = mysql_fetch_array($results) ?>
